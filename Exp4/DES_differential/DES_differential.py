...
# 两个明密文对类，主要用于差分攻击的一组（右半部分相同），包含很多获取异或的方法
...
class Attack(object):
    ...
    def attack(self):
        ...
        for i in range(8):
            ...
            if i == 4:
                print(self.couple1.get_test()[i])
                print(self.couple2.get_test()[i])
                print(self.couple3.get_test()[i])
    ...
    def enum_final_key(self):
        ...
        for i in range(len(PC2_rev)):
            if PC2_rev[i] == 100:
                omit_index.append(i)
        # print(omit_index)
        # 对每种可能的密钥进行加密，这里没有复用DES加密代码（因为接口不合）
...
if __name__ == '__main__':
    ...
    couple1 = Couple_Pair(Message_Cipher_Pair(message_11, cipher_11), Message_Cipher_Pair(message_12, cipher_12))
    couple2 = Couple_Pair(Message_Cipher_Pair(message_21, cipher_21), Message_Cipher_Pair(message_22, cipher_22))
    couple3 = Couple_Pair(Message_Cipher_Pair(message_31, cipher_31), Message_Cipher_Pair(message_32, cipher_32))
