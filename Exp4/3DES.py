...
# 无非就是按流程走三遍
def TriDES_encrypt(src_message, key1, key2):
    cipher1 = encrypt(src_message, generate_key(key1))
    cipher2 = encrypt(cipher1, generate_key(key2)[::-1])
    cipher3 = encrypt(cipher2, generate_key(key1))
    return cipher3

def TriDES_decrypt(src_cipher, key1, key2):
    message1 = encrypt(src_cipher, generate_key(key1)[::-1])
    message2 = encrypt(message1, generate_key(key2))
    message3 = encrypt(message2, generate_key(key1)[::-1])
    return message3

def useage():
    print("Useage: [--message] message [--cipher] cipher [--key(1|2|3)] key [--rounds] rounds(default 16)")
    sys.exit()
...
if __name__ == '__main__':
    ...
    try:
        ...
        have_cipher = False
        opts, args = getopt.getopt(sys.argv[1:], "hm:c:k1:k2:r:", 
            ["help", "message=", "cipher=", "key1=", "key2=", "rounds="])
        for opt, arg in opts:
            if opt in ("-h", "--help"):
                useage()
                ...
                useage()
        if (False in have_key or not (have_cipher or have_message)):
            print("More arguments are required.")
            useage()
        if (have_message):
            print("3DES ENCRYPTOR")
            print("=" * 20)
            print("Your message is: " + message)
            print("Your keys are: " + key1 + "    " + key2)
            cipher_generated = TriDES_encrypt(message, key1, key2)
            print("Encrypt finished. Your cipher is: " + cipher_generated)
            print("")
        if (have_cipher):
            print("3DES_DECRYPTOR")
            print("=" * 20)
            print("Your cipher is: " + cipher)
            print("Your keys are: " + key1 + "    " + key2)
            message_generated = TriDES_decrypt(cipher, key1, key2)
